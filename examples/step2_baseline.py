...
from pythia.vision import BaselineConfig, run_baseline_step2
...
def main() -> None:
    ...
    run_name = args.run_name or time.strftime("step2A_%Y%m%d_%H%M%S")
    ...
    try:
        row = run_baseline_step2(pipeline=pipeline, cfg=cfg, output_dir=out_dir, run_name=run_name)
    finally:
        pipeline.stop()
