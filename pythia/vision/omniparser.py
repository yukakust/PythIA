...
def _import_omniparser_utils(omni_path: str):
    ...
    try:
        ...
        return get_yolo_model, get_caption_model_processor, get_som_labeled_img
...
def run_omniparser_step2(
    *,
    ...
    get_yolo_model, get_caption_model_processor, get_som_labeled_img = _import_omniparser_utils(cfg.omni_path)
    ...
    for frame in pipeline.frames_all(start_from_latest=True):
        ...
        # For mode 1: use OmniParser's OCR. Not implemented yet.
        if cfg.mode == 1:
            raise NotImplementedError("OmniParser mode=1 (Omni OCR) is not implemented yet")
